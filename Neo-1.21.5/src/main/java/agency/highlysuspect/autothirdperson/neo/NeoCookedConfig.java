...
public class NeoCookedConfig implements CookedConfig {
	public NeoCookedConfig(ConfigSchema schema) {
		ModConfigSpec.Builder forge = new ModConfigSpec.Builder();
		...
		schema.accept(new ConfigSchema.Visitor() {
			...
			public <T> void visitProperty(ConfigProperty<T> prop) {
				...
				if(defaultValue instanceof Integer) {
					...
					ModConfigSpec.ConfigValue<Integer> forgeInt = forge.define(prop.name(), defaultInt, mkValidator(prop));
					modProps.put(prop, forgeInt);
				} else if(defaultValue instanceof Boolean) {
					boolean defaultBool = (Boolean) defaultValue;
					ModConfigSpec.ConfigValue<Boolean> forgeBool = forge.define(prop.name(), defaultBool, mkValidator(prop));
					modProps.put(prop, forgeBool);
				} else {
					String defaultString = prop.write(defaultValue); //using my own serializer
					ModConfigSpec.ConfigValue<String> forgeString = forge.define(prop.name(), defaultString, mkValidator(prop));
					modProps.put(prop, forgeString);
				}
			}
		});
	...
	public final ModConfigSpec modSpec;
	
	private final Map<ConfigProperty<?>, ModConfigSpec.ConfigValue<?>> modProps = new HashMap<>();
	...
	public <T> T load(ConfigProperty<T> prop) {
		Object frog = modProps.get(prop).get();
